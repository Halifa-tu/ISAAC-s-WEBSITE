"""
LandMark Realty — Flask + Neon PostgreSQL
"""
...
# Vercel filesystem is read-only — use /tmp
...
def init_db():
    ...
    if cur.fetchone()['c'] == 0:
        for s in [
            ('house','Elegant 4-Bedroom Family Home','Spacious modern home in a secure neighbourhood with fitted kitchen and master en-suite.','GHS 850,000','East Legon, Accra',4,3,'320 sqm',None,None,'available',1),
            ('room','Self-Contained Studio Room','Neat tiled self-contained room with private bath and kitchenette.','GHS 1,200/mo','Tema, Community 25',1,1,'45 sqm',None,None,'available',0),
            ('land','Prime Residential Plot','Fenced 1-acre plot with Indenture and Site Plan ready.','GHS 400,000','Kasoa, Central Region',None,None,'1 acre','Residential','Indenture, Site Plan','available',1),
            ('house','Modern 3-Bedroom Townhouse','Newly built townhouse with solar panels and CCTV security.','GHS 620,000','Tema, Community 18',3,2,'220 sqm',None,None,'available',0),
            ('room','Furnished 2-Bedroom Apartment','Fully furnished with AC, DSTV and WiFi included.','GHS 4,500/mo','Airport Residential, Accra',2,2,'90 sqm',None,None,'available',1),
            ('land','Commercial Land — Main Road','Half-acre on a busy road, ideal for shops or offices.','GHS 750,000','Spintex Road, Accra',None,None,'0.5 acre','Commercial','Indenture','available',0),
        ]:
            cur.execute("""INSERT INTO listings(type,title,description,price,location,bedrooms,
    ...
    if cur.fetchone()['c'] == 0:
        for r in [
            ('Kwame Acheampong','Homebuyer, Accra',5,'Found my dream home in East Legon within 2 weeks. Professional and genuinely helpful.'),
            ('Abena Boateng','Business Owner, Kumasi',5,'Submitted a request and had 3 perfect options within days. Exceptional service!'),
            ('Emmanuel Owusu','Investor, Tema',5,'Bought a plot in Kasoa with all documents intact. Fast and stress-free.'),
        ]:
...
def get_setting(key, default=''):
    row = db_one("SELECT value FROM settings WHERE key=%s", (key,))
...
def listing_to_dict(row):
    d = dict(row)
    imgs = db_all("SELECT id, filename FROM listing_images WHERE listing_id=%s ORDER BY sort_order", (d['id'],))
    d['images'] = [{'id': r['id'], 'filename': r['filename']} for r in imgs]
    return d

def fmt_dt(dt_val):
    try:
        if isinstance(dt_val, datetime): return dt_val.strftime('%d %b %Y, %I:%M %p')
        return datetime.strptime(str(dt_val)[:19], '%Y-%m-%d %H:%M:%S').strftime('%d %b %Y, %I:%M %p')
...
@app.route('/')
def index():
    featured     = [listing_to_dict(r) for r in db_all(
    ...
    return render_template('index.html', featured=featured,
                           total_active=total_active, agent=get_agent(), reviews=reviews)
...
def about():
    ...
    return render_template('about.html', agent=get_agent(), total_active=total_active, reviews=reviews)
...
def property_request():
    if request.method == 'POST':
        db_run("""INSERT INTO property_requests
...
def admin_signup():
    ...
    if request.method == 'POST':
        ...
        if db_one("SELECT id FROM admin_users WHERE email=%s", (email,)):
...
def admin_reset_password(token):
    ...
    if request.method == 'POST':
        ...
        db_run("UPDATE admin_users SET password=%s, reset_token=NULL WHERE id=%s",
...
def admin_dashboard():
    ...
    recent = db_all("SELECT * FROM property_requests ORDER BY created_at DESC LIMIT 5")
    return render_template('admin/dashboard.html', stats=stats, recent_requests=recent, agent=get_agent())

@app.route('/admin/listings')
@admin_required
def admin_listings():
    rows = db_all("SELECT * FROM listings ORDER BY created_at DESC")
    return render_template('admin/listings.html',
        listings=[listing_to_dict(r) for r in rows], agent=get_agent())
...
def _save_listing(listing_id):
    t    = request.form.get('type','house')
    titl = request.form.get('title','').strip()
    desc = request.form.get('description','').strip()
    pr   = request.form.get('price','').strip()
    loc  = request.form.get('location','').strip()
    bed  = request.form.get('bedrooms') or None
    bath = request.form.get('bathrooms') or None
    sz   = request.form.get('size','').strip()
    lu   = request.form.get('land_use','').strip()
    ld   = request.form.get('land_docs','').strip()
    st   = request.form.get('status','available')
    feat = 1 if request.form.get('featured') else 0
    if not titl or not pr:
    ...
    if listing_id:
        db_run("""UPDATE listings SET type=%s,title=%s,description=%s,price=%s,location=%s,
            ...
            WHERE id=%s""", (t,titl,desc,pr,loc,bed,bath,sz,lu,ld,st,feat,listing_id))
    else:
        cur = db_run("""INSERT INTO listings(type,title,description,price,location,bedrooms,
            bathrooms,size,land_use,land_docs,status,featured)
            VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id""",
            (t,titl,desc,pr,loc,bed,bath,sz,lu,ld,st,feat))
        listing_id = cur.fetchone()['id']
    order = db_val("SELECT COUNT(*) FROM listing_images WHERE listing_id=%s", (listing_id,))
    for f in request.files.getlist('images'):
        if f and f.filename and allowed_file(f.filename):
            ext = f.filename.rsplit('.',1)[1].lower()
            fname = f"{uuid.uuid4().hex}.{ext}"
            f.save(os.path.join(app.config['UPLOAD_FOLDER'], fname))
            db_run("INSERT INTO listing_images(listing_id,filename,sort_order) VALUES(%s,%s,%s)",
...
def admin_listing_delete(id):
    for img in db_all("SELECT filename FROM listing_images WHERE listing_id=%s", (id,)):
        try: os.remove(os.path.join(app.config['UPLOAD_FOLDER'], img['filename']))
        except: pass
    db_run("DELETE FROM listings WHERE id=%s", (id,))
...
@admin_required
def admin_requests():
    rows = db_all("SELECT * FROM property_requests ORDER BY created_at DESC")
...
def admin_request_reply(id):
    db_run("UPDATE property_requests SET admin_reply=%s WHERE id=%s", (request.form.get('reply',''), id))
...
def admin_request_delete(id):
    db_run("DELETE FROM property_requests WHERE id=%s", (id,))
...
@admin_required
def admin_messages():
    rows = db_all("SELECT * FROM messages ORDER BY created_at DESC")
...
def admin_message_reply(id):
    db_run("UPDATE messages SET admin_reply=%s WHERE id=%s", (request.form.get('reply',''), id))
    flash('success:Reply saved.')
...
def admin_message_delete(id):
    db_run("DELETE FROM messages WHERE id=%s", (id,))
...
@admin_required
def admin_reviews():
    rows = db_all("SELECT * FROM reviews ORDER BY approved ASC, created_at DESC")
...
def admin_review_approve(id):
    db_run("UPDATE reviews SET approved=1 WHERE id=%s", (id,))
...
def admin_review_delete(id):
    db_run("DELETE FROM reviews WHERE id=%s", (id,))
...
@admin_required
def admin_users():
    users = db_all("SELECT * FROM admin_users ORDER BY created_at DESC")
...
def admin_user_delete(id):
    if id == session.get('admin_id'):
        ...
        db_run("DELETE FROM admin_users WHERE id=%s", (id,))
...
def admin_settings():
    if request.method == 'POST':
        ...
        if new_pw:
            ...
            db_run("UPDATE admin_users SET password=%s WHERE id=%s",
