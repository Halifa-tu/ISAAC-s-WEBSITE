...
"""
import os, uuid, secrets, re, base64
...
DB_URL        = os.environ.get('DATABASE_URL', 'postgresql://neondb_owner:npg_D2Zuhakv8YNK@ep-damp-darkness-amse7s7j.c-5.us-east-1.aws.neon.tech/neondb?sslmode=require')
...
def init_db():
    ...
    for s in [
        ...
        """CREATE TABLE IF NOT EXISTS listing_images (
            ...
            filename TEXT, image_data TEXT, sort_order INTEGER DEFAULT 0)""",
...
def listing_to_dict(row):
    d = dict(row)
    d['images'] = [{'id':r['id'],'filename':r['filename'],'image_data':r.get('image_data','')}
                   for r in db_all("SELECT id,filename,image_data FROM listing_images WHERE listing_id=%s ORDER BY sort_order",(d['id'],))]
    return d
...
@app.route('/db-test')
def db_test():
    import traceback, json as j
    results = {}
    try:
        row = db_one("SELECT NOW() as t")
        results['db_time'] = str(row['t']) if row else 'no result'
        results['status'] = 'OK'
    except Exception as e:
        results['error'] = str(e)
        results['trace'] = traceback.format_exc()
    return '<pre>' + j.dumps(results, indent=2) + '</pre>'
...
def serve_image(img_id):
    ...
    row = db_one("SELECT image_data, filename FROM listing_images WHERE id=%s", (str(img_id),))
...
def _save_listing(lid):
    ...
    for f in request.files.getlist('images'):
        if f and f.filename and allowed_file(f.filename):
            ...
            db_run("INSERT INTO listing_images(listing_id,filename,image_data,sort_order) VALUES(%s,%s,%s,%s)",(str(lid),fname,data_url,str(order)))
            order+=1
...
def admin_listing_delete(id):
    db_run("DELETE FROM listing_images WHERE listing_id=%s",(str(id),))
